...
program
    ...
    .option("--spill <dir>", "store memory-heavy solver sets on disk (LMDB) under the given scratch directory instead of the V8 heap, to avoid out-of-memory on large programs")
    .option("--spill-cache-size <number>", "max total members of disk-spilled sets cached in the heap (default: 4000000; use with --spill)", parsePositiveInt)
    .option("--spill-threshold <number>", "set size above which a set is moved from the heap to the disk store (default: 256; use with --spill)", parsePositiveInt)
